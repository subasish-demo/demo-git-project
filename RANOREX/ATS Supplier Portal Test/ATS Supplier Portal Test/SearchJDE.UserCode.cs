...
{
    public partial class SearchJDE
    {
        /// <summary>
        ...
        public void ValidateLedgerTableEntries(string orderNumber, string lineNumber, string userName, string buyerNumber)
        {
        	try 
        	{
        		var xmlFilePath = @"\\ca01a9001\pgmis\Deployment_DEV\Ranorex\Supplier_Portal\JDE\SQLLedgerTableResultsReopen.xml";
        		...
        		Report.Info("Querying JDE ledger table F43199 with Ordernumber: " + orderNumber + " and linenumber: " + lineNumber + " to see if user: " + userName + " has entries for buyernumber: " + buyerNumber + " for todays date and next status of 120 and last status of 110");
        		...
        		Ranorex.Validate.IsTrue("120" == table.Rows[0][3].ToString(), "Validating Next Status is 120 of the JDE ledger table F43199 line item.  The validation @ValidateRESULT@!");
				Ranorex.Validate.IsTrue("110" == table.Rows[0][4].ToString(), "Validating Last Status is 110 of the JDE ledger table F43199 line item.  The validation @ValidateRESULT@!");        		
        	} 
        	catch (Exception) 
        	{
        ...
        {	
            ...
            do
            {
            	...
            	result = SPCollection.SQLQueryJDELastStatus(userRef,supplierSo,quantity, false);
            	if(result == "110")
            	{
            	...
            	{
            		Report.Info("Result is: " + result + ".  Result is not '110' in JDE, retrying in 1 minute...");
            	}
            	i++;
